...
public class DispatcherServlet extends HttpServlet {
    ...
    /**
     * 当 Servlet容器创建当前的Servlet实例后，会自动调用init(ServletConfig)方法
     */
    @Override
    public void init() throws ServletException {
        ...
        for (Class<?> controllerClass : controllers) {
            try {
                Object controllerInstance = controllerClass.getConstructor().newInstance(); //反射到controller
                //依次处理每个Method:
                        ...
                        for (Class<?> parameterClass : method.getParameterTypes()) {
                            if (!supportPostParameterTypes.contains(parameterClass)) {
                                if (requestBodyClass == null) {
                                    ...
                                    throw new UnsupportedOperationException(
                                            "Unsupported return type: " + parameterClass + "for method: " + method
                                    );
                                }
                            }
                        }
    ...
    private static final Set<Class<?>> supportGetParameterTypes = Set.of(int.class, long.class, boolean.class, String.class,
            HttpServletRequest.class, HttpServletResponse.class, HttpSession.class);

    private static final Set<Class<?>> supportPostParameterTypes = Set.of(
            HttpServletRequest.class, HttpServletResponse.class, HttpSession.class);
}
